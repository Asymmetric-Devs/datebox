...
export default function AlbumsScreen() {
  ...
  // Extract and scope albums to the currently selected group
  const allAlbums: Album[] = unwrapAlbums(albumsQuery.data);
  const albums: Album[] = useMemo(
    () => allAlbums.filter((album) => !groupId || album.groupId === groupId),
    [allAlbums, groupId],
  );

  // ── Pending albums (being generated by AI) ──
  ...
  useEffect(() => {
    ...
    for (const pending of pendingAlbums) {
      if (currentTitles.has(pending.title.trim().toLowerCase())) {
        console.log("✅ Álbum generado detectado, quitando preview:", pending.title);
